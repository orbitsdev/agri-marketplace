<div class="ml-4 flow-root lg:ml-6 relative group">
    <a 
        href="{{ route('cart.view', ['name' => Auth::user()->fullNameSlug()]) }}" 
        ...
        @if ($itemCount > 0)
            <span
                class="absolute top-[-4px] right-0 translate-x-2 -translate-y-2 bg-eucalyptus-700 text-white text-xs font-medium rounded-full px-2">
                {{ $itemCount }}
            </span>
        @endif
    </a>
